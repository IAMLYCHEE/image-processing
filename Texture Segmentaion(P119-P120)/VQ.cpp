...
vector2d initWithRand(vector2d data,int N,int D,int K) {
	vector2d centroids;
	for (int j = 0; j < K; j++) {	
...
vector2d initWithGA(vector2d data,int D, int N,int K) {
	//regenerate samples with GA
	vector2d newData = regenerate(data, 50, D, N);
	vector2d centroids = initWithRand(newData, 50, D,K);
	return centroids;
}
...
vector<int> VQ::classify(int initType)
{
	...
	if (initType == 1) {
		...
		centroids = initWithRand(data,N,D,K);
	}
	else {
		centroids = initWithGA(data, D, N,K);
	}
