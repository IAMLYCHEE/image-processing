...
unsigned char * Image::rgb2gray(unsigned char * image, int width, int height)
{
	int size = width * height * 3;
	unsigned char *gray3 = (unsigned char*)malloc((sizeof(unsigned char) * width * height));
	for (int i = 0; i < width * height * 3; i = i + 3) {
		gray3[i / 3] = (unsigned char)((int)(0.21*image[i] + 0.72*image[i + 1] + 0.07*image[i + 2]));
	}
	return gray3;
}
...
vector<double> Image::genLoGKernel(int ws, double sigma) {
	double pi = 3.14159265;
	int center = -ws / 2;
	double c1 = 1/(pi* pow(sigma, 4));
	...
	for (int i = 0; i < ws; i++) {
		for (int j = 0; j < ws; j++) {
			...
			double ss = x*x + y * y;
			double value = c1 * (ss / c2 - 1) *exp(-(ss / c2));
			filter.push_back(value);
		}
	}
	//normalize
	...
	for (int i = 0; i < filter.size(); i++) {
		filter[i] = filter[i] - sum/filter.size();
	}
	double scale = pow(10, sigma);
	for (int i = 0; i < filter.size(); i++) {
...
double Image::getmean(unsigned char* data,int width,int height) {
	double sum = 0;
...
double Image::getABSmean(vector<double> data) {
	double sum=0;
...
vector<double> Image::filter2d(unsigned char* imageData, vector <vector<double>> kernel, int width, int height,double dataMean) {
	//reshape vector
	vector<double> kernel1d;
	for (auto e : kernel) {
		for (auto ee : e) {
			kernel1d.push_back(ee);
		}
	}
	...
	for (int i = windowSize / 2; i < height-windowSize/2; i++) {
		for (int j = windowSize / 2; j < width-windowSize/2; j++) {
			...
			for (int m = -windowSize / 2; m < windowSize / 2 + 1; m++) {
				for (int n = -windowSize / 2; n < windowSize / 2 + 1; n++) {
					blockData.push_back((double)imageData[(i + m)*width + j + n]-dataMean);
				}
			}
			double productResult = innerProduct(blockData, kernel1d, kernel1d.size());
			response.push_back(productResult);
			blockData.clear();
		}
	}
	return response;
...
unsigned char * Image::inverse(unsigned char * data, int width, int height, int channel)
{
	int dataSize = width * height * channel;
	unsigned char* inversed = (unsigned char*)malloc(sizeof(unsigned char)*dataSize);
	for (int i = 0; i < dataSize; i++) {
		inversed[i] = 255 - data[i];
	}
	return inversed;
}

vector<double> Image::filter2d(unsigned char * imageData, vector<vector<double>> kernel, int width, int height)
{
	//reshape vector
	vector<double> kernel1d;
	for (auto e : kernel) {
	...
	for (int i = windowSize / 2; i < height - windowSize / 2; i++) {
		for (int j = windowSize / 2; j < width - windowSize / 2; j++) {
			...
			for (int m = -windowSize / 2; m < windowSize / 2 + 1; m++) {
				for (int n = -windowSize / 2; n < windowSize / 2 + 1; n++) {
					blockData.push_back((double)imageData[(i + m)*width + j + n]);
				}
			}
			double productResult = innerProduct(blockData, kernel1d, kernel1d.size());
			response.push_back(productResult);
...
vector<double> Image::filter2d(unsigned char * imageData, vector<double> kernel, int width, int height,bool edge)
{
	cout << "filtering..." << endl;
	int windowSize = (int)sqrt(kernel.size());
		...
		for (int j = 0; j < width ; j++) {
			...
			for (int m = -windowSize / 2; m < windowSize / 2 + 1; m++) {
				for (int n = -windowSize / 2; n < windowSize / 2 + 1; n++) {
					...
					int posH = j + n;
					if (posV < 0) posV = 0;
					if (posV >= height) posV = height - 1;
					if (posH < 0) posV = 0;
					if (posH >= width) posH = width - 1;
					blockData.push_back((double)imageData[posV*width + posH]);
				}
			}
			double productResult = innerProduct(blockData, kernel, kernel.size());
			response.push_back(productResult);
			blockData.clear();
			//plot something interesting
			//int progress = (i*width + j)*100 / (height * width);
		//	cout << "[";
		//	for (int i = 0; i < progress/4; i++) {
		//		cout << "#";
		//	}
		//	for (int i = progress / 4; i < 25; i++) {
		//		cout << " ";
		//	}
		//	cout << "]    " << progress << "%" << '\r';
		}
...
vector<double> Image::normalize2UCRange(vector<double> imageData) {
	vector<double> result = imageData;
	sort(result.begin(), result.end());
	double min = result[0];
	double max = result[result.size() - 1];
	for (int i = 0; i < imageData.size(); i++) {
		result[i] = (imageData[i] - min) / (max - min) * 255.0;
	}
	return result;
}

vector<double> Image::getABSResponse(vector<double> response) {
	vector<double> result = response;
	for (int i = 0; i < response.size(); i++) {
		result[i] = abs(response[i]);
	}
	return result;
}

unsigned char * Image::castV2UC(vector<double> imageData)
{
	unsigned char* result = (unsigned char*)malloc(sizeof(unsigned char)* imageData.size());
